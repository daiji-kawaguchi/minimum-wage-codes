...
cap log close;
cap eststo clear;
log using ${path}\log\cs_reg.log,replace;
clear;
...
forvalues s=1/2{;
eststo: qui regress emp lmw ${control} ${fix} if sex==`s'&age>=19&age<=24, cluster(prefecture);
qui estadd ysumm;
estimates store emp1924_`s';
eststo: qui regress emp lmw ${control} ${fix} if sex==`s'&age>=25&age<=59, cluster(prefecture);
qui estadd ysumm;
estimates store emp2559_`s';
eststo: qui regress emp lmw ${control} ${fix} if sex==`s'&age>=60&age<=64, cluster(prefecture);
qui estadd ysumm;
...
esttab using "${path}\out\crossreg.tex", replace label b(2) t(2) 
...
forvalues s=1/2{;
eststo: qui regress lf lmw ${control} ${fix} if sex==`s'&age>=19&age<=24, cluster(prefecture);
qui estadd ysumm;
eststo: qui regress lf lmw ${control} ${fix} if sex==`s'&age>=25&age<=59, cluster(prefecture);
qui estadd ysumm;
eststo: qui regress lf lmw ${control} ${fix} if sex==`s'&age>=60&age<=64, cluster(prefecture);
qui estadd ysumm;
};
...
forvalues s=1/2{;
eststo: qui regress lhour lmw ${control} ${fix} if sex==`s'&age>=19&age<=24, cluster(prefecture);
eststo: qui regress lhour lmw ${control} ${fix} if sex==`s'&age>=25&age<=59, cluster(prefecture);
eststo: qui regress lhour lmw ${control} ${fix} if sex==`s'&age>=60&age<=64, cluster(prefecture);
};
esttab, b(2) t(2) keep(lmw) se nostar compress mtitles("M19-24" "M25-59" "M60-64" "F19-24" "F25-59" "F60-64");
esttab using "${path}\out\crossreg.tex", append label b(2) t(2) keep(lmw) se nostar compress fragment;
eststo clear;
...
forvalues s=1/2{;
eststo: qui regress size lmw ${control} ${fix} if sex==`s'&age>=19&age<=24, cluster(prefecture);
eststo: qui regress size lmw ${control} ${fix} if sex==`s'&age>=25&age<=59, cluster(prefecture);
eststo: qui regress size lmw ${control} ${fix} if sex==`s'&age>=60&age<=64, cluster(prefecture);
};
esttab, b(2) t(2) keep(lmw) se nostar compress mtitles("M19-24" "M25-59" "M60-64" "F19-24" "F25-59" "F60-64");
*esttab using "${path}\out\crossreg.tex", append label b(2) t(2) keep(lmw) se nostar compress fragment;
eststo clear;
